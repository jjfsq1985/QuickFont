...
namespace QuickFont
{
    ...
    public class QFont : IDisposable
    {
        ...
        private IFont _dynamicFont;
        private QFontBuilderConfiguration _dynamicConfig;
        private readonly HashSet<char> _builtChars = new HashSet<char>();
        ...
        /// <summary>
        /// Ensures all characters in <paramref name="text"/> are in the font atlas,
        /// rebuilding it incrementally if new characters are found.
        /// Only works when the QFont was constructed from a font file or IFont with a config.
        /// </summary>
        public void EnsureChars(string text)
        {
            if (_dynamicFont == null || _dynamicConfig == null) return;

            bool hasNew = false;
            foreach (char c in text)
            {
                if (c != ' ' && c != '\n' && c != '\r' && !_builtChars.Contains(c))
                {
                    hasNew = true;
                    break;
                }
            ...
            _dynamicConfig.CharSet = new string(new List<char>(_builtChars).ToArray());
            var oldData = FontData;
            FontData = BuildFont(_dynamicFont, _dynamicConfig, null);
            oldData.Dispose();
        }
        ...
        {
            float fontScale = 1f;
            IFont font = GetFont(fontPath, size, style, config?.SuperSampleLevels ?? 1, fontScale);
        ...
        {
            float fontScale = 1f;
            IFont font = new FreeTypeFont(fontData, size, style, config?.SuperSampleLevels ?? 1, fontScale);
            FontName = font.ToString();
            InitialiseQFont(font, config);
        }
        ...
        {
            if (data == null && font != null && config != null)
            {
                _dynamicFont = font;
