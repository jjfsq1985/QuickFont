...
namespace QuickFont
{
    ...
    internal class Builder
    {
        ...
        private Bitmap CreateInitialBitmap(IFont font, SizeF maxSize, int initialMargin, out QFontGlyph[] glyphs, TextGenerationRenderHint renderHint, List<SizeF> sizes)
        {
            ...
            int cellWidth = (int)Math.Ceiling(maxSize.Width) + 2 * initialMargin;
            int cellHeight = (int)Math.Ceiling(maxSize.Height) + 2 * initialMargin;

            // 计算每行最多字符数
            int cols = Math.Max(1, MAX_TEXTURE_SIZE / cellWidth);
            int rows = (_charSet.Length + cols - 1) / cols;
            int actualWidth = Math.Min(cols * cellWidth, MAX_TEXTURE_SIZE);
